...
import { IconHome, IconSearch, IconTables, IconCalendar, IconMap,
  IconDevises, IconSettings, IconSettingsUser, IconFinance,
...
export const Sidebar: React.FC = () => {
  ...
  const settingsHandler = () => {
    setSettings({
      ...
      icon : settings.isActive === false ? 'nav__arrow' : 'nav__arrow nav__arrow-down',
    })
  }
      ...
      <ul className='nav'>
        { sidebar.map((option: ISidebarItem, index : number) => {
            if (option.selection) {
              ...
              <Link className={setActiveLink(option.to)} to={option.to} key={index}>
                <img src={option.icon} className='nav__icon'/>
                <li className='nav__item'>{option.title}</li>
